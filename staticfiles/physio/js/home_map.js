(function () {
  ...
  async function openLocationWorkflow(marker, loc) {
    ...
    async function loadSlots() {
      const dateStr = document.getElementById(`p_date_${loc.id}`).value;
      ...
      // Event delegation for timeslot buttons
      slotDiv.onclick = (e) => {
    ...
    async function loadConsultants(timeStr) {
      ...
      const url = `${cfg.consultantsUrl}?location_id=${loc.id}&date=${encodeURIComponent(dateStr)}&time=${encodeURIComponent(timeStr)}`;
      const data = await fetchJSON(url);
    ...
    async function fetchJSON(url) {
      ...
      // If Django returned an error page (HTML), don't try res.json()
